...
looking full while saying something false". That habit is why 38,000 lines of
test are readable, and it is the one convention a new file must keep.

`1,839` `def test_` functions across 40 test modules, plus three scripts and a
fixture package.
...
| `test_loadtest.py` | 609 | the root `loadtest.py` client's own gates |
...
Empty, and required. Without it `tests.fixtures`, `tests.load` and
`tests.model_sweep` are not importable packages, and half the suite imports one
of the three by dotted name.
...
box being busy. Run it with `pytest tests/test_load.py -m slow`.

## `test_loadtest.py`

37 tests on the repo-root `loadtest.py`. That script is a client of the published
