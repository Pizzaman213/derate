...
def unknown_node_message(node_id: str, available: list[str]) -> str:
    """The sentence for a node id that is not enrolled.

    A message rather than a response, because placement refusals carry a param
    and an override block that `error_response` has no slot for. The prose
    shape is `unknown_model`'s on purpose: name what does exist, so a client
    can correct itself.
    """
    listing = ", ".join(f"'{name}'" for name in available) if available else "none"
    return (
        f"The machine '{node_id}' is not in this cluster. "
        f"Enrolled machines: {listing}."
    )
