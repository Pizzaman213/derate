 ...
 */
export function DegreeFields({ degrees, onChange, effective, recommended, overruled }: Props) {
  ...
  return (
    <>
      {field('tensor_parallel', 'pb-tp', 'TP')}
      {field('pipeline_parallel', 'pb-pp', 'PP')}
      {degrees ? (
        <div className="fld">
          {/* Occupies the label row so the control lines up with the fields. */}
          <label htmlFor="pb-degrees-reset">&nbsp;</label>
          <button id="pb-degrees-reset" type="button" onClick={() => onChange(null)}>
            Reset
          </button>
        </div>
      ) : null}
